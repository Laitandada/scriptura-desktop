...
export async function POST(req: Request) {
  try {
    ...
    const data = {
      ...
      fontFamily: state.settings.verseSettings?.fontFamily || 'var(--font-inter)',
      fontSize: state.settings.verseSettings?.fontSize || 90,
      overlayOpacity: state.settings.overlayOpacity,
      showReference: state.settings.showReference,
      alignment: state.settings.verseSettings?.alignment || 'center',
      fontWeight: state.settings.verseSettings?.fontWeight || 'bold',
      textShadow: state.settings.verseSettings?.textShadow || 'medium',
      textOutline: state.settings.verseSettings?.textOutline || 'none',
      outlineColor: state.settings.verseSettings?.outlineColor || '#000000',
      textColor: state.settings.verseSettings?.textColor || '#ffffff',
    };
